...
export const TechnicalIndicators = ({ snapshot }: { snapshot: MarketSnapshot }) => {
  return (
    <div className="bg-white/5 border border-white/10 p-6 h-full rounded-2xl space-y-6">
      <h3 className="text-xs font-black uppercase tracking-[0.3em] text-muted-foreground opacity-50">Technical Pulse</h3>
      
      <div className="space-y-4">
        <div className="p-4 rounded-xl bg-white/5 border border-white/5">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] uppercase text-muted-foreground flex items-center gap-2"><TrendingUp size={12}/> MA(20) Status</span>
            <span className={snapshot.Close > snapshot.MA_20 ? "text-green-400" : "text-red-400"} text-xs font-bold>
              {snapshot.Close > snapshot.MA_20 ? "ABOVE" : "BELOW"}
            </span>
          </div>
          <div className="text-lg font-mono font-bold tracking-tighter">₹{snapshot.MA_20.toFixed(2)}</div>
        </div>

        <div className="p-4 rounded-xl bg-white/5 border border-white/5">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] uppercase text-muted-foreground flex items-center gap-2"><Activity size={12}/> RSI Analysis</span>
            <span className="text-blue-400 text-xs font-bold uppercase">{snapshot.RSI_14 > 70 ? 'Overbought' : snapshot.RSI_14 < 30 ? 'Oversold' : 'Stable'}</span>
          </div>
          <div className="text-lg font-mono font-bold tracking-tighter">{snapshot.RSI_14.toFixed(2)}</div>
        </div>
      </div>
    </div>
  );
