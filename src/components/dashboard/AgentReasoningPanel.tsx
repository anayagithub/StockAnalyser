...
import { useState } from "react";
import {
  Brain, TrendingUp, Activity, BarChart3, Newspaper, AlertTriangle,
  CheckCircle2, ChevronDown, ChevronUp, Lightbulb, Shield,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { Badge } from "@/components/ui/badge";
import { Switch } from "@/components/ui/switch";
...
interface AgentReasoningPanelProps {
  ticker: string; // Dynamic ticker prop
  className?: string;
}

// Hardcoded data store mapped by ticker
const TICKER_DATA: Record<string, { steps: ReasoningStep[]; simple: string; recommendation: string }> = {
  INFY: {
    recommendation: "BUY",
    simple: "The stock price is going UP consistently—like a ball rolling uphill. Recent AI partnerships with NVIDIA have boosted sentiment, and technicals show institutional accumulation.",
    steps: [
      { id: "price", category: "Price Analysis", icon: <TrendingUp className="h-4 w-4" />, title: "Upward price trend detected", analysis: "Trading above 20-day and 50-day MAs for 15 sessions.", signal: "supports", details: "Price at ₹1,847.50 (+3.2% above 20-day MA)." },
      { id: "sentiment", category: "News Sentiment", icon: <Newspaper className="h-4 w-4" />, title: "AI Partnership Momentum", analysis: "Expansion with NVIDIA is viewed as a major long-term moat.", signal: "supports", details: "60% of recent news is positive." },
    ]
  },
  RELIANCE: {
    recommendation: "HOLD",
    simple: "Reliance is seeing mixed signals. While retail expansion is strong, global crude oil volatility is putting pressure on the O2C margins. Better to wait for clarity.",
    steps: [
      { id: "price", category: "Price Analysis", icon: <TrendingUp className="h-4 w-4" />, title: "Consolidation Phase", analysis: "Stock is moving sideways within a tight range of ₹2,850 - ₹2,980.", signal: "neutral", details: "Currently trading at ₹2,910." },
      { id: "risk", category: "Risk Assessment", icon: <AlertTriangle className="h-4 w-4" />, title: "O2C Margin Pressure", analysis: "Refining margins are expected to be soft this quarter.", signal: "caution", details: "Global Brent crude volatility exceeds 12%." },
    ]
  }
};

export const AgentReasoningPanel = ({ ticker, className }: AgentReasoningPanelProps) => {
  const [simpleMode, setSimpleMode] = useState(false);
  const [expandedSteps, setExpandedSteps] = useState<string[]>([]);

  // Get data for current ticker or fallback to default
  const data = TICKER_DATA[ticker] || TICKER_DATA["INFY"];
  ...
  const signalConfig = {
    supports: { icon: CheckCircle2, color: "text-positive", bg: "bg-positive/10", border: "border-positive/30", label: "Supports" },
    caution: { icon: AlertTriangle, color: "text-warning", bg: "bg-warning/10", border: "border-warning/30", label: "Caution" },
    neutral: { icon: Shield, color: "text-muted-foreground", bg: "bg-secondary", border: "border-border", label: "Neutral" },
  };
    ...
    <div className={cn("glass-card p-6", className)}>
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold">Agent Reasoning: {ticker}</h3>
            <p className="text-sm text-muted-foreground">AI analysis explaining the current outlook</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Switch id="simple-mode" checked={simpleMode} onCheckedChange={setSimpleMode} />
          <Label htmlFor="simple-mode" className="text-sm cursor-pointer">
            <Lightbulb className="h-4 w-4 inline mr-1" /> Explain like I'm a beginner
          </Label>
        </div>
      </div>
      ...
      {simpleMode ? (
        <div className="rounded-lg bg-secondary/50 p-6 animate-fade-in">
          <p className="text-sm leading-relaxed text-foreground">{data.simple}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {data.steps.map((step) => {
            ...
            return (
              <Collapsible key={step.id} open={isExpanded} onOpenChange={() => toggleStep(step.id)}>
                <div className={cn("rounded-lg border transition-all", config.border, isExpanded && config.bg)}>
                  <CollapsibleTrigger asChild>
                    <button className="flex w-full items-center justify-between p-4 text-left hover:bg-secondary/30 transition-colors rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg", config.bg, config.color)}>{step.icon}</div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-medium text-muted-foreground">{step.category}</span>
                            <Badge variant="outline" className={cn("gap-1 text-[10px]", config.color)}><SignalIcon className="h-2.5 w-2.5" />{config.label}</Badge>
                          </div>
                          <p className="font-medium mt-0.5">{step.title}</p>
                        </div>
                      </div>
                  ...
                  <CollapsibleContent className="px-4 pb-4 pl-11">
                    ...
                    {step.details && (
                      <div className="mt-3 rounded bg-background/50 p-3 text-xs border border-border/50">
                        <strong className="text-foreground">Technical Details:</strong> {step.details}
                      </div>
                    )}
                  </CollapsibleContent>
                </div>
      ...
      {/* Final Recommendation */}
      <div className={cn("mt-6 rounded-xl border p-6", 
        data.recommendation === "BUY" ? "bg-positive/10 border-positive/30" : "bg-warning/10 border-warning/30"
      )}>
        <h4 className={cn("text-lg font-semibold", data.recommendation === "BUY" ? "text-positive" : "text-warning")}>
          Final Recommendation: {data.recommendation}
        </h4>
        <p className="mt-2 text-sm text-foreground leading-relaxed">
          The AI agent currently rates {ticker} as a <strong>{data.recommendation}</strong> based on aggregated technical and news signals.
        </p>
      </div>
    </div>
  );
